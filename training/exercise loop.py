...
# 1. Total Penjualan Mingguan per Produk
...
# 2. Rata-rata Penjualan Harian Semua Produk
...
# 3. Produk dengan Penjualan Tertinggi di Hari Tertentu
while True:
    try:
        hari_input = int(input("Masukkan nomor hari (1-7): "))
        if 1 <= hari_input <= 7:
            break
        else:
...
produk_terlaris_hari_itu = []

for produk in data_penjualan:
    nama_produk = produk['nama_produk']
    penjualan_hari_ini = produk['penjualan_harian'][hari_index]
    if penjualan_hari_ini > penjualan_tertinggi_hari_itu:
        penjualan_tertinggi_hari_itu = penjualan_hari_ini
        produk_terlaris_hari_itu = [nama_produk]
    elif penjualan_hari_ini == penjualan_tertinggi_hari_itu:
        produk_terlaris_hari_itu.append(nama_produk)

if produk_terlaris_hari_itu:
    print(f"Produk dengan penjualan tertinggi di hari ke-{hari_input}: {', '.join(produk_terlaris_hari_itu)}")
else:
